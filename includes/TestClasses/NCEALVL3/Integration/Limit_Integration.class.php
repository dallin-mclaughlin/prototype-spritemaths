<?php
    ...
    class LimitIntegration extends Question{   
        ...
        function poly(){
            $midPoint = random_int(-10,10);
            $lowerLimit = $midPoint - random_int(1,5);
            $higherLimit = $midPoint + random_int(1,5);
            $variable = 'x';
            $degree = random_int(0,5);
            $coefficients = [];
            $difCoefficients = [];
            for($i = 0; $i <= $degree; $i++){
            ...
            $value = 0;
            for($i = 0; $i < count($coefficients)-1;$i++){
                $value += $coefficients[$i]*pow($higherLimit, count($coefficients)-1-$i);
                $value -= $coefficients[$i]*pow($lowerLimit, count($coefficients)-1-$i);
            }
            ...
            $this->addToBlurb('\mathrm{Evaluate \ the \ integral }');
            $this->setQuestion('\int_{'.$lowerLimit.'}^{'.$higherLimit.'}'.$difExpression.' \ \mathrm{dx}');
